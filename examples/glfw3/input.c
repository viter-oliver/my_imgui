...
void _glfwMemHandle(_GLFWwindow* window,unsigned long tag,unsigned long memlen,void*pmem)
{
	if (window->callbacks.mem)
	{
		window->callbacks.mem(window,tag,memlen,pmem);
	}
}
...
void _glfwDeviceState(int state, wchar_t* pdev_name)
{
	if (_glfw.callbacks.usb_device_monitor)
	{
		_glfw.callbacks.usb_device_monitor(state,pdev_name);
	}
}
...
GLFWAPI GLFWmemfun glfwSetMemCallback(GLFWwindow* handle,GLFWmemfun memfun)
{
	_GLFWwindow* window = (_GLFWwindow*)handle;
	assert(window != NULL);

	//_GLFW_REQUIRE_INIT_OR_RETURN(NULL);
	_GLFW_SWAP_POINTERS(window->callbacks.mem, memfun);
	return memfun;
}
...
GLFWAPI void glfwUsbDevicStateCallback(GLFWnotifyfun cbfun)
{
	_GLFW_REQUIRE_INIT_OR_RETURN(NULL);
	_GLFW_SWAP_POINTERS(_glfw.callbacks.usb_device_monitor, cbfun);
	return cbfun;
}
