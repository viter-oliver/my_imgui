                                   ...
                                   WPARAM wParam, LPARAM lParam)
{
    ...
    if (!window)
    {
        ...
        switch (uMsg)
        {
            case WM_DEVICECHANGE:
            {
				...
				//printf("device wparam=0x%x dbh->dbch_devicetype=0x%x\n", wParam, dbh->dbch_devicetype);
                ...
                {      
					...
					{
						DEV_BROADCAST_DEVICEINTERFACE* pdi = (DEV_BROADCAST_DEVICEINTERFACE*)lParam;
						wprintf(L"in dbcc_name=%s\n", pdi->dbcc_name);
						_glfwDeviceState(1, pdi->dbcc_name);
                ...
                else if (wParam == DBT_DEVICEREMOVECOMPLETE)
                {
					...
					{
						DEV_BROADCAST_DEVICEINTERFACE* pdi = (DEV_BROADCAST_DEVICEINTERFACE*)lParam;
						wprintf(L"out dbcc_name=%s\n", pdi->dbcc_name);
						_glfwDeviceState(0, pdi->dbcc_name);
            ...
            case WM_COPYDATA:
            {
                 ...
                 _glfwMemHandle( window, pCopyData->dwData, pCopyData->cbData, pCopyData->lpData );
                 break;
            }
        }
    ...
    switch (uMsg)
    {
        ...
        case WM_DROPFILES:
        {
            ...
            const int count = DragQueryFileA(drop, 0xffffffff, NULL, 0);
            ...
            for (i = 0;  i < count;  i++)
            {
                const UINT length = DragQueryFileA(drop, i, NULL, 0);
                char* buffer = calloc(length + 1, sizeof(char));

                DragQueryFileA(drop, i, buffer, length + 1);
                paths[ i ] = buffer; //_glfwCreateUTF8FromWideStringWin32( buffer );
