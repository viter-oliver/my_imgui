...
void keep_state_trans_on()
{
     for( auto& istm : g_mstate_manager )
     {
		...
		if (stm._play_state==en_play_tran)
		{
			...
			auto delta_tm = dur_mills.count() - cur_trans._start_time;//note?��ostart_time2?��2?��o?��o?��??|��??��???��o??���衧|����o����??o��??a��o?
			if (delta_tm>0)
			{
				...
				assert(stm._cur_from<stm._prop_value_list.size()&&"invalid from");
				assert(stm._cur_to<stm._prop_value_list.size()&&"invalid to");
