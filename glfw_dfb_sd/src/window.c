    ...
    if (ctxconfig.client != GLFW_NO_API)
    {
				...
				ctxconfig.source == GLFW_NATIVE_CONTEXT_API)
		{
		 printf("%s::%d\n",__FUNCTION__,__LINE__);
            ...
            window->dfb.width=width;
			window->dfb.height=height;
			
    		window->dfb.native = fbCreateWindow(native_display, 0, 0,  width,  height );
			if (!_glfwInitEGL()||!_glfwCreateContextEGL(window, &ctxconfig, &fbconfig))
			{
...
GLFWAPI float glfwGetWindowOpacity(GLFWwindow* handle)
{
    ...
    return 1.f;//_glfwPlatformGetWindowOpacity(window);
}
