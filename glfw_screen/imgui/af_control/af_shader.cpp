...
void af_shader::refresh_viarable_list()
{
	...
	for (idx = 0; idx < count; idx++)
	{
		...
		cout<<"Attribute "<<idx <<" Type:"<<type<< "Name:"<< name<<endl;
...
bool af_shader::vertex_att_pointer(initializer_list<string> att_name_list)
{
	...
	{
		const auto& iatt = _att_list.find(iname);
		if (iatt==_att_list.end())
		{
			cout<<"fail to find attribute name:"<< iname<<endl;
			return false;
		}
...
bool af_shader::uniform(string unf_name, float* pvalue)
{
	...
	if (tt == _unf_list.end())
	{
		cout<<"fail to find attr:"<< unf_name<<endl;
		return false;
	}
	auto& unif = tt->second;
	switch (unif._variable_type)
